...
The Blaker 'exact' CI may be obtained as follows from {DescTools}. An identical result is obtained from `PropCIs::blakerci(),` but the SAS PROC FREQ result (0.170101, 0.307308) only matches to 4 decimal places.

```{r}
options(digits=8)
# cardx package blaker method 
DescTools::BinomCI(36, 154, method=c("blaker", "clopper-pearson")) |>
  as_tibble(rownames = "Method")
```
