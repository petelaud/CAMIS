...
-   2 sample proportions ($p_1$ and $p_2$) and you want a CI for a contrast parameter, such as the difference in the 2 proportions (risk difference, $RD = p_1 - p_2$), the ratio (relative risk, $RR = p_1 / p_2$), or the odds ratio ($OR = p_1 (1-p_2) / (p_2 (1-p_1))$).
    ...
    -   If estimates from more then one set of samples is to be combined using a stratified estimate, for example contrast estimates from a clinical trial with stratified randomisation, or combining results from multiple trials in a meta-analysis.

In some cases, data might be collected on events ocurring over a specified exposure time (for example adverse events), in which case a Poisson model would be appropriate for estimating exposure-adjusted event rates.
...
## Normal Approximation Methods (Also known as Wald Methods)
...
For the contrast parameters, score methods were derived for independent proportions by Miettinen & Nurminen and others in the 1980s[@miettinen1985][@Mee1984][@koopman1984], and related methods for paired data followed later[@tango1998][@tango1999][@nam2002][@tang2003]. Miettinen & Nurminen also produced formulae for the Poisson event rate contrasts. In each case, the variance is a function of both $p_1$ and $p_2$, or of $\theta$ and a 'nuisance parameter', which is eliminated using the maximum likelihood estimate for $p_2$ (or for the paired case, the cell probability $p_{21}$) for the given value of $\theta$.
...
This class of methods was popularised by Newcombe for the RD contrast, and is therefore labelled as the Newcombe method in current SAS PROC FREQ documentation. It has since been generalised to other binomial and Poisson contrasts and for paired data (though not implemented in SAS), and renamed as the Method of Variance Estimates Recovery (MOVER). It is also referred to as the 'Square-and-add' approach.
...
Here we provide an overview of the packages containing methods for the different analysis types. Further detail of the specific methods offered and agreement between packages can be found on the relevant pages.
...
Methods are included for one-sample proportions and limited options for 2 independent samples, but {cardx} currently does not have a method for 2 matched proportions.

**The {PropCIs} package** produces a selection of CIs for single proportion, and estimation of all contrasts for independent proportions and paired proportions (except paired RR). The {PropsCIs} package generally requires just the number of events (numerator number of successes) & total number of subjects (denominator) as an input dataset.

**The {ratesci} package** contains functions to produce a variety of CIs for a single binomial proportion or Poisson event rate, and all contrasts of independent proportions/rates or paired proportions. It has a particular emphasis on classes of methods that can be applied across different analysis types, in particular the Asymptotic Score methods (with or without skewness correction) which come with naturally consistent hypothesis tests (for association or non-inferiority), and also caters for analysis of stratified datasets. The full range of MOVER methods is also included.

**The {Hmisc} package** contains the binconf() function, which produces CIs for a single proportion only.

**The {RBesT} package** contains a function (BinaryExactCI()) to produce the Clopper-Pearson CI only.

**The {ExactCIdiff} package** produces 'exact' CIs for RD for two independent or paired proportions.

**The {DescTools} package** has functions which produce CIs for single proportions and all contrasts of two proportions (independent samples only).
